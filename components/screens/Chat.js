...
export default function Chat({ route, navigation }) {
  ...
  const getChatMessages = async (user, chatId) => {
    try {
      ...
      const response = await fetch(`${URLS.BASE}/messages/${chatId}`, {
        method: 'GET',
        headers: {
  ...
  const sendMessage = async (user, chat) => {
    try {
      const response = await fetch(`${URLS.BASE}/messages`, {
        method: 'POST',
        body: JSON.stringify({
          sender: user.id,
          content: chat.msg,
          chatId: chatId,
          image: chat.image ? await RNFS.readFile(chat.image, 'base64') : null,
        }),
        headers: {
          'Content-type': 'application/json; charset=UTF-8',
          Accept: 'application/json',
          ...
          <View style={STYLES.buttonContainer}>
            <TouchableOpacity
              ...
              <Text style={STYLES.textCaption}>
                {type == 'CameraType.front' ? 'Selfie mode' : 'Back Camera'}
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={STYLES.button}
        ...
        <ScrollView
          ...
          {chats?.map((chatItem, index) => (
            <Amessage
              style={[
                STYLES.message,
                chatItem.senderId === user.id ? STYLES.you : STYLES.other,
              ]}
              key={index.toString()}
              chat={chatItem}
            />
